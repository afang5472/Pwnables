...
BLOCK_SIZE = 16
PADDING = '\x00'
pad = lambda s: s + (BLOCK_SIZE - len(s) % BLOCK_SIZE) * PADDING #fulfill padding element to 16 quants with \x00.
...
def sanitize(arg): #allow only 0-9, a-z , -_
	for c in arg:
...
def AES128_CBC(msg):
	cipher = AES.new(key, AES.MODE_CBC, iv) #AES three element, key , iv and mode.
	return EncodeAES(cipher, msg)

def request_auth(id, pw):
	packet = '{0}-{1}-{2}'.format(id, pw, cookie) #format id-pw-cookie
