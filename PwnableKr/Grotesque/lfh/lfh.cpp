...
public:
	...
	Bucket(UINT m_class, UINT mode){ //instantiate certain type of bucket according to type.
		this->next = NULL;
...
public:
	...
	void* Alloc(UINT size){
			UINT chunk_class = (size/0x10) * 0x10 + 0x10; //padding size
...
BOOKS* load_file(const char* fname){
	...
	while(1){
		...
		r = read(fd, pbook->content, len); //fake-heap overflow!
		if(r!=len){
			break;
		}
