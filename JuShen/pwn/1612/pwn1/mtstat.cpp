...
void mtstat::drawTile2 ( int event ) const
{
    ...
    {
        printf ( "\e[34m\e[4mBKEY\e[24m\e[39m" );
    }
...
{
    printf ( "\e[H\e[2J" );
    printf ( "HP: \e[33m\e[1m%8d\e[21m\e[39m AT: \e[35m\e[1m%8d\e[21m\e[39m DF: \e[34m\e[1m%8d\e[21m\e[39m MF: \e[32m\e[1m%8d\e[21m\e[39m \n\n"
             "G:  \e[36m\e[1m%8d\e[21m\e[39m T: %2d YK: %2d BK: %2d RK: %2d P: %2d B: %2d\n\n"
             "X: %2d  Y: %2d FLOOR: %2d    FFF: %d reg='%c' Score: \e[33m\e[1m%4d\e[21m\e[39m"
             "\n\nU: (%4d, %4d, %4d, %4d)\n\n",
             brave_hp, brave_at, brave_df, brave_mf, brave_gold, brave_tokens, brave_yellowkey, brave_bluekey, brave_redkey, brave_pickaxe, brave_bomb, brave_x, brave_y, brave_floor, fanfare_flag, reg, score(), unit_hp, unit_at, unit_df, unit_mf );
    for ( int i = 0; i < MAPSIZE; i++ )
    {
        for ( int j = 0; j < MAPSIZE; j++ )
        {
            ...
            if ( maps[brave_floor][i][j] == 0 )
            {
                int coord = brave_floor * MAPSIZE * MAPSIZE + i * MAPSIZE + j;
                int coord2;
                int f2;
                char comment[64] = {};
                for ( int i = 0, s = portals.size(); i < s; i++ )
                {
                    if ( portals[i][0] == coord )
                    {
                        coord2 = portals[i][1];
                        f2 = coord2 / 225;
                        sprintf ( comment, "\e[46m%3dF\e[49m", f2 );
                        break;
                    }
                    if ( portals[i][1] == coord )
                    {
                        coord2 = portals[i][0];
                        f2 = coord2 / 225;
                        sprintf ( comment, "\e[46m%3dF\e[49m", f2 );
        ...
        for ( int j = 0; j < MAPSIZE; j++ )
        {
            ...
            if ( maps[brave_floor][i][j] == 0 )
            {
                ...
                for ( int i = 0, s = portals.size(); i < s; i++ )
                {
                    ...
                    {
                        coord2 = portals[i][1];
                        x2 = coord2 % 15, y2 = ( coord2 / 15 ) % 15;
                        sprintf ( comment, "\e[46m\e[3m%2d\e[23m\e[1m%2d\e[21m\e[49m", x2, y2 );
                        break;
                    }
                    if ( portals[i][1] == coord )
                    {
...
void mtstat::drawMobData ( bool showall ) const
{
    puts ( "================================================" );
    bool mobs[0x80] = {};
    if ( !showall )
    {
        for ( int i = 0; i < MAPSIZE; i++ )
        {
            for ( int j = 0; j < MAPSIZE; j++ )
            {
                mobs[maps[brave_floor][i][j] - 0x80] = true;
            }
        }
    }
    for ( int i = 0, s = enemies.size(); i < s; i++ )
    {
        if ( showall || mobs[i] )
        {
            int spec_num = enemies[i][4];
            char spec_buf[128] = {};
            if ( spec_num & CNMASK )
            {
                sprintf ( spec_buf, "\e[36m\e[1m%dx Chained\e[21m\e[39m ", ( spec_num & CNMASK ) + 1 );
            }
            if ( spec_num & MA )
            {
                strcat ( spec_buf, "\e[35m\e[1mMagic\e[21m\e[39m " );
            }
            if ( spec_num & SD )
            {
                strcat ( spec_buf, "\e[1mSolid\e[21m " );
            }
            if ( spec_num & SA )
            {
                strcat ( spec_buf, "\e[33m\e[1mSpeedy\e[21m\e[39m " );
            }
            if ( spec_num & BF )
            {
                strcat ( spec_buf, "\e[31m\e[1mInvulnerable\e[21m\e[39m " );
            }
            printf ( "Mob %7d :           Dam \e[31m\e[1m%19d\e[21m\e[39m\n%s\nHP \e[33m\e[1m%8d\e[21m\e[39m AT \e[35m\e[1m%8d\e[21m\e[39m DF \e[34m\e[1m%8d\e[21m\e[39m G  \e[36m\e[1m%8d\e[21m\e[39m\n================================================\n", i, dam ( i ), spec_buf, enemies[i][0], enemies[i][1], enemies[i][2], enemies[i][3] );
        }
    }
}
