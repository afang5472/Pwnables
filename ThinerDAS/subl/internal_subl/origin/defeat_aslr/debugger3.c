...
static void step(struct control_block* cb)
{
	...
	printf("\e[32msubl [0x%08x]->0x%08x, [0x%08x]->0x%08x, [0x%08x](branch), [0x%08x](next_pc)\n", ph_a, A, ph_b, B, ph_c, new_pc);
	...
	if(A<B){
		//if A<B, branch 
		printf("next_pc will be: 0x%08x\n", ph_c);	
	}else{
		printf("next_pc will stick to: 0x%08x\n", new_pc);
	}
	}
