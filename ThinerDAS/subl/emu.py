...
class Environ(object):
    ...
    def __init__(self, memory, flag):
        ...
        #init procedure here.
        self.memory = [None]*(2**10) #1024's slots.
        self.rMEMSTAT = 0
    ...
    def step(self):
        pc = self.rPC
        op_a = self.read_abs(pc & ((1 << 22)-1), 4) # operator bin_a
        op_b = self.read_abs((pc+4) & ((1 << 22)-1), 4) # operator bin_b
        orig_op_c = self.read_abs((pc+8) & ((1 << 22)-1), 4) #operator bin_c

        self.rPC = (pc+12) & ((1 << 22)-1) #pc point to next 12bytes start position.
        n = [4, 4, 2, 1][orig_op_c & 3] # find orig_op_c's least 2 bit, check to spawn an n.
        op_c = orig_op_c & -4 #last two bit set zero
    ...
    def read_abs(self, address, n, forced=True):
        ...
        if address < 128: #if address < 0x7f, then it refers to accessing read_dma ops
